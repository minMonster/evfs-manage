    ...
    <div>
      <div class="bg-white padding" v-if="rule !== ''" style="margin-bottom: 20px;">
        ...
        <RadioGroup class="approval" v-model="rule">
          <Row>
            <Col span="6">
            <Radio :disabled="rule !== 0" :label="0">任意一个数据存管域管理员签批</Radio>
            </Col>
            <Col span="6">
            <Radio :disabled="rule !== 100" :label="100">1/3数据存管域管理员同时签批</Radio>
            </Col>
            <Col span="6">
            <Radio :disabled="rule !== 200" :label="200">2/3数据存管域管理员同时签批</Radio>
            </Col>
            <Col span="6">
            <Radio :disabled="rule !== 300" :label="300">所有数据存管域管理员同时签批</Radio>
            </Col>
          </Row>
        </RadioGroup>
...
export default {
  ...
  methods: {
    ...
    init () {
      api.pbqrc({
        reviewType: 'storage_manage_rule',
    ...
    async agreeRule () {
      ...
      let data = await cApi.pbgen({
        'method': 'DSDomainRuleAgreeContractTxReq',
        'jsBody': JSON.stringify(jsBody)
      }).then(res => {
        return {
          hexTxBody: res.hexTxBody,
